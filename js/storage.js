/*
  ...
  Mantiene compatibilidad con la estructura actual guardada.
*/
...
function readLocalStorageItem(key, contextMessage = "leer datos", fallback = null) {
  try {
...
function writeLocalStorageItem(key, value, contextMessage = "guardar datos", notifyUser = false) {
  try {
    ...
    if (notifyUser && typeof window !== "undefined" && typeof window.alert === "function") {
      window.alert(
        "No se pudieron guardar los cambios en el navegador. Lo más reciente podría perderse si recargas la página."
      );
    }
